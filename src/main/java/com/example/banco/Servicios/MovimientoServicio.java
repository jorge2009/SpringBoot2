...
public class MovimientoServicio {
    ...
    public boolean  guardarMovimiento(Movimientos movimiento){
        ...
        cue.setSaldo_inicial(movimiento.getCuenta().getSaldo_inicial());
        ...
        String tmovimiento=movimiento.getTipo_movimiento();
        double saldo=0;
        ...
        switch (tmovimiento) {
            case "deposito":
                ...
                saldoInicial=movimiento.getCuenta().getSaldo_inicial();
                //cue.setSaldo_inicial(saldoInicial);
              
                nsaldo=saldo+movimiento.getValor()+saldoInicial;
                formateador.format(nsaldo);
                saldo=nsaldo;
                resultado=true;
                estado=true;
                break;
            case "retiro":
                saldo=movimiento.getSaldo();
                if(valor>1000){
                System.out.print("Exede de limite diario de retiro");
                resultado=false;
                }else
                {
                saldo=movimiento.getSaldo();
                if(saldo==0){
                    System.out.println("No puede realizar retiro su saldo es cero");
                    resultado=false;
                }else{
                saldoInicial=movimiento.getCuenta().getSaldo_inicial();
                nsaldo=saldo-movimiento.getValor()-saldoInicial;
                formateador.format(nsaldo);
                if(nsaldo<0){
                    System.out.println("No puede realizar retiro su saldo es menor a cero");
                    resultado=false;
